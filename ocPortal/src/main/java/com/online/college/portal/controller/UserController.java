...
import com.online.college.common.storage.FastDFSUtils;
...
public class UserController {
	...
	public ModelAndView index(TailPage<UserFollowStudyRecord> page){
		...
		for(UserFollowStudyRecord item : page.getItems()){
			if(StringUtils.isNotEmpty(item.getHeader())){
				item.setHeader(FastDFSUtils.getFileUrl(item.getHeader()));
			}
		}
	...
	public ModelAndView info(){
		...
		if(null != authUser && StringUtils.isNotEmpty(authUser.getHeader())){
			authUser.setHeader(FastDFSUtils.getFileUrl(authUser.getHeader()));
		}
	...
	@ResponseBody
	public String saveInfo(AuthUser authUser) throws Exception{
